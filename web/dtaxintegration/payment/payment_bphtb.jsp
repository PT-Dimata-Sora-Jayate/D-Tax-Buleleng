...
<%@page import="com.dimata.dtaxintegration.session.bphtb.DTaxIntegrationMonitorBphtb"%>
<%@page import="com.dimata.webclient.AppSettingBphtb"%>
<%@page import="java.util.Calendar"%>
<%@page import="com.dimata.dtaxintegration.entity.payment.PaymentBphtbRaversalIprotax"%>
<%@page import="com.dimata.dtaxintegration.entity.payment.PstPaymentBphtbRaversalIprotax"%>
<%@page import="com.dimata.dtaxintegration.entity.payment.PaymentBphtbIprotax"%>
<%@page import="com.dimata.dtaxintegration.entity.payment.PstPaymentBphtbIprotax"%>
...
<%!//proses yang akan ditampilkan
    ...
    public String drawListDataPaymentBpd(int iCommand, Vector objectClass, long gradeId) {
        ...
        ctrlist.addHeader("STATUS", "200px");//9
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            rowx = new Vector(1, 1);
			String status = "";
			if (payment.getStsReversal().equals("0")){
				status = "Bayar";
			} else {
				status = "Reversal";
			}
            ...
            rowx.add(""+payment.getNama());//5
            rowx.add(""+payment.getTagihan());//8
            rowx.add(""+Formater.formatDate(transaksiDate, "yyyy-MM-dd"));//9
            rowx.add(""+status);//10
    ...
    public String drawListDataPaymentDispenda(int iCommand, Vector objectClass, Vector objectClass2, long gradeId) {
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            //Simpatda simpatda = (Simpatda) objectClass.get(i);
            PaymentBphtbIprotax paymentBphtb = (PaymentBphtbIprotax) objectClass.get(i);
            count=count+1;
            rowx = new Vector(1, 1);
            
            rowx.add(""+count);//1
            rowx.add(""+paymentBphtb.getKdProvinsi()+paymentBphtb.getKdDati2()+paymentBphtb.getThbBphtb()+paymentBphtb.getBlnBphtb()+paymentBphtb.getTglBphtb()+paymentBphtb.getNoUrutBphtb()+paymentBphtb.getIndeksBphtb());//2
            rowx.add(""+paymentBphtb.getNamaWP());//3
            rowx.add(""+FRMHandler.userFormatStringDecimal(paymentBphtb.getBphtbSdhBayar()));//4
            rowx.add("Bayar");//4
            total=total+paymentBphtb.getBphtbSdhBayar();
            lstData.add(rowx);
        }

		for (int i = 0; i < objectClass2.size(); i++) {
            
            //Simpatda simpatda = (Simpatda) objectClass.get(i);
            PaymentBphtbRaversalIprotax paymentBphtb = (PaymentBphtbRaversalIprotax) objectClass2.get(i);
            count=count+1;
            rowx = new Vector(1, 1);
...
<% //proses yang tidak akan ditampilkan hanya diserver side
    ...
    if(iCommand==Command.POSTING){
        DTaxIntegrationMonitorBphtb dtaxIntegration = new DTaxIntegrationMonitorBphtb();
		if(AppSettingBphtb.TYPE_APP_BACKOFFICE == AppSettingBphtb.APP_IPROTAX){
             dtaxIntegration.inputPaymentBphtbIprotax(startDate,"");
         }else{
	...
	Vector listPaymentReversal = new Vector();
        whereClause = "TGL_PEMBAYARAN LIKE '%"+oDate+"%'";
        listPayment = PstPaymentBphtbIprotax.list(0, 0, whereClause+" AND NO_TRANSAKSI_BYR_BANK IS NOT NULL", "");
        listPaymentReversal = PstPaymentBphtbRaversalIprotax.list(0, 0, whereClause, "");
                            ...
                            <div class="col-xs-12">
                               <div class="box">
                                   ...
                                   <div class="box-header">
                                       ...
                                       <% if(userSession.checkPrivG1G2ObjCommand(AppObjInfo.G1_PROSES_INTEGRASI,AppObjInfo.G2_BPHTB,AppObjInfo.OBJ_DOWNLOAD_PAYMENT_BPHTB, AppObjInfo.COMMAND_DOWNLOAD)){ %>
                        ...
                        <div class="row">
                            <div class="col-xs-12">
                               <div class="box">
                                   ...
                                   <%=drawListDataPaymentDispenda(iCommand, listPayment,listPaymentReversal, oidDriver)%>
                                </div><!-- /.box -->
                            </div>
                        </div>         
                    </form>
