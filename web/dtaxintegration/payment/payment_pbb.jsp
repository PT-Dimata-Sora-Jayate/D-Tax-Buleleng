...
<%!//proses yang akan ditampilkan
    ...
    public String drawListDataPaymentPhrBpd(int iCommand, Vector objectClass, long gradeId) {
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            rowx.add(""+FRMHandler.userFormatStringDecimal((Math.abs(Double.parseDouble(payment.getTagihan()))-Double.parseDouble(payment.getDenda().replaceAll(",","")))));//8
            rowx.add(""+FRMHandler.userFormatStringDecimal(Double.parseDouble(payment.getDenda().replaceAll(",",""))));//9
            rowx.add(""+FRMHandler.userFormatStringDecimal(Double.parseDouble(payment.getTagihan())));//10
            rowx.add(""+status);
    ...
    public String drawListDataPaymentPhrDispenda(int iCommand, Vector objectClass, long gradeId) {
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            if(paymentPbb.getIdPaymentBank() > 0){
                count=count+1;
                rowx = new Vector(1, 1);
                ...
                if(AppSetting.TYPE_APP_BACKOFFICE == AppSetting.APP_IPROTAX|| AppSetting.TYPE_APP_BACKOFFICE == AppSetting.APP_IPROTAX_V2){
                    rowx.add(""+paymentPbb.getNopIpprotax());//3
                }else{
                    rowx.add(""+paymentPbb.getNop());//3
                }
                rowx.add(""+paymentPbb.getThnPajakSppt());//4
                            rowx.add(""+FRMHandler.userFormatStringDecimal((Math.abs(paymentPbb.getJmlSpptYgDibayar()-paymentPbb.getDendaSppt()))));//8
                rowx.add(""+FRMHandler.userFormatStringDecimal(paymentPbb.getDendaSppt()));//9
...
<% //proses yang tidak akan ditampilkan hanya diserver side
    ...
    if(iCommand==Command.POSTING){
        DTaxIntegrationMonitor dtaxIntegration = new DTaxIntegrationMonitor();
         if(AppSetting.TYPE_APP_BACKOFFICE == AppSetting.APP_IPROTAX || AppSetting.TYPE_APP_BACKOFFICE == AppSetting.APP_IPROTAX_V2){
             dtaxIntegration.inputPaymentPBBIproTax(startDate,"");
         }else{
    ...
    if(AppSetting.SQL_VERSION==AppSetting.DBSVR_ORACLE){
        whereClause = PstPaymentPbb.fieldNames[PstPaymentPbb.FLD_TGLPEMBAYARANSPPT]+" >= TO_DATE ('"+startDate+" 00:00:00','YYYY-MM-DD HH24:MI:SS')"
				+ " AND " + PstPaymentPbb.fieldNames[PstPaymentPbb.FLD_TGLPEMBAYARANSPPT]+" < TO_DATE ('"+startDate+" 23:59:00','YYYY-MM-DD HH24:MI:SS')";
		/*whereClause = PstPaymentPbb.fieldNames[PstPaymentPbb.FLD_TGLPEMBAYARANSPPT]+" = TO_DATE ('"+oDate+"','YYYY-MM-DD HH24:MI:SS')";*/
    }else{
        if(AppSetting.TYPE_APP_BACKOFFICE == AppSetting.APP_IPROTAX || AppSetting.TYPE_APP_BACKOFFICE == AppSetting.APP_IPROTAX_V2){
            whereClause = " TGL_PEMBAYARAN_SPPT BETWEEN '"+startDate+" 00:00:00' AND '"+startDate+" 23:59:00' AND NO_TRANSAKSI_BYR_SPPT_BANK != '0'";
        }
    }
    
    if(AppSetting.TYPE_APP_BACKOFFICE == AppSetting.APP_IPROTAX || AppSetting.TYPE_APP_BACKOFFICE == AppSetting.APP_IPROTAX_V2){
        listPaymentPBB = PstPaymentPbb.listIpprotax(0, 0, whereClause, PstPaymentPbb.fieldNames[PstPaymentPbb.FLD_THNPAJAKSPPT]);
    }else{
