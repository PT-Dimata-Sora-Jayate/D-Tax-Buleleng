...
<%!//proses yang akan ditampilkan
    ...
    public String drawListDataPaymentDispenda(int iCommand, Vector objectClass, long gradeId) {
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            //rowx.add("<center><button type=\"button\" class=\"btn btn-success\" onclick=\"javascript:cmdPosting('" +retribusi.getNoRekening()+"','"+retribusi.getTanggalPenerimaan()+"')\"></i> UPLOAD </button></center>");
            rowx.add("<center><input type=\"checkbox\" name=\"status_"+i+"\" value=\"'"+retribusi.getNoRekening()+"';'"+retribusi.getTanggalPenerimaan()+"'\" class=\"form-control\" size=\"10\"></center>");
...
<% //proses yang tidak akan ditampilkan hanya diserver side
    ...
    String whereClauseHistory = " INSTANSI='RETRIBUSI_GIANYAR' AND TAHUN="+tahunLog+" AND BULAN="+monthLog+" AND TANGGAL="+dateLog;
                  /*" INSTANSI='RETRIBUSI_GIANYAR' AND "+PstLogHistoryTransaksi.fieldNames[PstLogHistoryTransaksi.FLD_TGL_RETRIBUSI]
                  +" BETWEEN TO_DATE ('"+oDate+"','YYYY-MM-DD HH24:MI:SS')"
                  +" AND TO_DATE ('"+oDate+"','YYYY-MM-DD HH24:MI:SS')";*/
    
    String whereClauseViewHistory = " WHERE INSTANSI='RETRIBUSI_GIANYAR' AND "+PstLogHistoryTransaksi.fieldNames[PstLogHistoryTransaksi.FLD_TGL_RETRIBUSI]
                  +" BETWEEN TO_DATE ('"+oDate+"','YYYY-MM-DD HH24:MI:SS')"
                  +" AND TO_DATE ('"+oDate+"','YYYY-MM-DD HH24:MI:SS')";
